...
// Middleware
app.use(cors({
    origin: ["https://bristol-west.vercel.app", "http://localhost:5173"],
...
const User = mongoose.model('User', new mongoose.Schema({
    fullName: String, email: { type: String, unique: true }, phone: String, password: String, role: { type: String, default: 'user' }, createdAt: { type: Date, default: Date.now }
}));
const Quote = mongoose.model('Quote', new mongoose.Schema({
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
    zipCode: String, firstName: String, lastName: String, dob: String, address: String, unit: String, city: String, email: String, phone: String,
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (email === "admin@gmail.com" && password === "admin1234") {
            ...
            if (!admin) {
                admin = await new User({ fullName: "System Admin", email: "admin@gmail.com", password: await bcrypt.hash("admin1234", 10), role: "admin" }).save();
            }
